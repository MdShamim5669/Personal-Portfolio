...
export const SkillCategoryChart = ({ categoryTitle, skillsData = [] }) => {
  const categories = useMemo(() => skillsData.map((s) => s.name), [skillsData]);
  const dataValues = useMemo(() => skillsData.map((s) => s.proficiency), [skillsData]);

  const chartOptions = useMemo(() => {
    return {
      chart: {
        id: 'skillCategoryBarChart',
        type: 'bar',
        height: 280,
        toolbar: { show: false },
        animations: {
  ...
  }, [categories, dataValues, categoryTitle]);
